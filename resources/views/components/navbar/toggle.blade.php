<button type="button"
    ...
    @click="open = !open" :aria-expanded="open.toString()" aria-controls="mobile-navigation"
    :aria-label="open ? '{{ __('Cerrar menú') }}' : '{{ __('Abrir menú') }}'">
    <span class="sr-only" x-text="open ? '{{ __('Cerrar menú') }}' : '{{ __('Abrir menú') }}'"></span>
    <svg x-show="!open" x-cloak class="size-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
        aria-hidden="true">

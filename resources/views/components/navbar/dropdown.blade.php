@props(['label', 'href' => '#', 'items' => [], 'active' => false, 'variant' => 'desktop'])
...
<div x-data="{ dropdownOpen: {{ $active ? 'true' : 'false' }} }" @keydown.escape.stop="dropdownOpen = false" @click.outside="dropdownOpen = false"
    ...
    <div class="flex items-center gap-2">
        <a href="{{ $href }}" @if ($active) aria-current="page" @endif
            ...
            class="flex-1 rounded-sm px-1 py-2 text-sm font-medium transition-colors hover:text-amc-orange focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amc-orange focus-visible:ring-offset-2 focus-visible:ring-offset-amc-blue {{ $active ? 'text-amc-orange' : 'text-white/90' }}">
            {{ $label }}
        </a>

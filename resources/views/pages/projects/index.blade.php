<x-layouts::app :title="__('Proyectos | AMC Gestión de Riesgos')" :description="__('Conoce los proyectos y resultados prácticos de AMC Gestión de Riesgos.')">
    ...
    <main class="mx-auto max-w-7xl px-6 py-24 lg:px-8">
        ...
        @if ($projects->count() > 0)
            <div class="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($projects as $project)
                    <a href="{{ route('projects.show', $project->slug) }}" class="group block">
                        <article class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-zinc-200 transition hover:shadow-md dark:bg-zinc-800 dark:ring-zinc-700">
                            ...
                            <div class="p-6">
                                <h2 class="text-xl font-semibold text-amc-blue group-hover:text-amc-orange-text transition dark:text-white">
                                    {{ $project->title }}
                                </h2>
            ...
            @endif
        @else
            <div class="mt-12 rounded-lg border-2 border-dashed border-zinc-300 p-12 text-center dark:border-zinc-600">

<div>
    @if ($show)
        <div
            @class([
                "scrollbar max-h-96 min-h-96 w-full overflow-auto rounded-sm bg-gray-200 shadow-lg no-dark:text-white! no-dark:bg-slate-900!",
            ])
            @style([
            ...
            >
                @foreach ($users as $user)
                    <button
                        ...
                        class="flex w-full cursor-pointer items-center gap-x-4 p-2 transition hover:bg-gray-300 no-dark:hover:bg-slate-800! focus:border-0! focus:bg-gray-300! focus:outline-hidden"
                        type="button"
                    >

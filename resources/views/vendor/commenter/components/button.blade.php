...
@if ($type === "button")
    <button
        ...
        @endif
        type="button"
        {{ $attributes->merge(["class" => $class . $severityClass . "inline-flex hover:cursor-pointer justify-center items-center gap-x-2 font-semibold rounded-sm border border-transparent text-white disabled:opacity-50 no-dark:focus:outline-hidden no-dark:focus:ring-1 no-dark:focus:ring-gray-600"]) }}
        @style([$severityStyle])
    >
        {{ $slot }}
    </button>
